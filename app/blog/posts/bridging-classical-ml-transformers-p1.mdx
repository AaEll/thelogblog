...
Common values for B1 and B2 are `(0.9, 0.999)` (PyTorch defaults) or `(0.9, 0.98)` (common in [LLM pre-training publications](https://proceedings.neurips.cc/paper_files/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf)).
...
Regulaization needs to be "decoupled" because of RMSProp. In RMSProp (and Adam), a classical regularization term is a part of the loss,
 and when the gradient is applied that regularization term's gradient, as part of the loss gradient, is passed through the same adaptive RMSProp denominator.
