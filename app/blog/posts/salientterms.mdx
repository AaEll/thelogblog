...
Salient terms came into the spotlight after the March 2024 leak of Google's search algorithm factors [15].
83 of Google' s

Within the leaked terms, salient terms appears in two distinct factors SalientTermSet, . 

<p>SalientTerm can be two things depending on where this message is. When right under a SalientTermSet, it is a normalized term and weight pair, along with other term-level data. When under another SalientTerm message, it is a non-normalized original term (see original_term field).
...
*   `salience` (*type:* `number()`, *default:* `nil`) - salience is the importance of the term as a descriptor in [0, 1] (the higher the more important).
</p>
...
We can infer *salient terms* as a term weighting algorithm with the goal of highly weighting terms related to the subject of the document.
